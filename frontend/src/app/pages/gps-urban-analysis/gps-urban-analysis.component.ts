...
import { GpsRunFile, GPS_MODE_COLORS, UrbanTestSummary } from '../../models/gps-analysis.model';
...
function gpsToLocal(lat: number, lon: number, oLat: number, oLon: number): [number, number] {
  const R = 6371000, cos = Math.cos(oLat * Math.PI / 180);
  return [(lon - oLon) * Math.PI / 180 * R * cos, (lat - oLat) * Math.PI / 180 * R];
}
...
function buildRefIndex(pts: GeoPoint[]): RefIndex {
  const origin = pts[0];
  const xs: number[] = [], ys: number[] = [], cumArc = [0];
  for (const p of pts) { const [x, y] = gpsToLocal(p.lat, p.lon, origin.lat, origin.lon); xs.push(x); ys.push(y); }
  ...
  const cornerIndices: number[] = [], cornerSAlongs: number[] = [];
  for (let i = 1; i < xs.length - 1; i++) {
    const dx1 = xs[i] - xs[i-1], dy1 = ys[i] - ys[i-1], dx2 = xs[i+1] - xs[i], dy2 = ys[i+1] - ys[i];
    const l1 = Math.sqrt(dx1*dx1+dy1*dy1), l2 = Math.sqrt(dx2*dx2+dy2*dy2);
    if (l1 < 1 || l2 < 1) continue;
    const angle = Math.acos(Math.max(-1, Math.min(1, (dx1*dx2+dy1*dy2)/(l1*l2)))) * 180 / Math.PI;
    if (angle >= 40) { cornerIndices.push(i); cornerSAlongs.push(cumArc[i]); }
  }
  return { xs, ys, cumArc, totalArc, origin, cornerIndices, cornerSAlongs };
}

function crossTrackToRef(p: GeoPoint, idx: RefIndex): { error: number; sAlong: number } {
  const [px, py] = gpsToLocal(p.lat, p.lon, idx.origin.lat, idx.origin.lon);
  let minErr = Infinity, bestS = 0;
  for (let i = 0; i < idx.xs.length - 1; i++) {
    const ax = idx.xs[i], ay = idx.ys[i], bx = idx.xs[i+1], by = idx.ys[i+1];
    const dx = bx - ax, dy = by - ay, len2 = dx*dx + dy*dy;
    const t = len2 > 0 ? Math.max(0, Math.min(1, ((px-ax)*dx + (py-ay)*dy) / len2)) : 0;
    const err = Math.sqrt((px - ax - t*dx)**2 + (py - ay - t*dy)**2);
    if (err < minErr) { minErr = err; bestS = idx.cumArc[i] + t * (idx.cumArc[i+1] - idx.cumArc[i]); }
  }
...
function computeSpeedJitter(pts: { lat: number; lon: number; time?: Date }[]): number {
  const spds: number[] = [];
...
async function parseRefFile(file: File): Promise<GeoPoint[]> {
  ...
  if (coordEl?.textContent) {
    return coordEl.textContent.trim().split(/\s+/).flatMap(chunk => {
      const [lonS, latS] = chunk.split(',');
      const lon = parseFloat(lonS), lat = parseFloat(latS);
      return isFinite(lat) && isFinite(lon) ? [{ lat, lon }] : [];
    });
  }
  throw new Error('No se encontraron puntos. Usa .gpx con <trkpt> o .kml con <LineString>.');
}
...
interface UrbanModeAnalytics {
  ...
  rmse: number; p95: number; maxErr: number;
  buildingPct: number; cornerErr: number; speedJitter: number;
  sampleHz: number; nSamples: number;
}

type AptLevel = 'elite' | 'high' | 'standard' | 'marginal' | 'no-apto' | 'sin-ref';
interface UrbanAptitude { level: AptLevel; label: string; sublabel: string; failing: string[]; }
...
export class GpsUrbanAnalysisComponent implements OnInit, OnDestroy {
  ...
  set mapContainerRef(el: ElementRef<HTMLDivElement> | undefined) {
    if (el && !this.leafletMap) this.ngZone.runOutsideAngular(() => this.initLeafletMap(el.nativeElement));
  }
  ...
  readonly presetModeNames = ['GPS Solo', 'Todos los Sistemas', 'Todos + Multibanda', 'SatIQ', 'UltraTrac'];
  ...
  async onRefFileSelected(event: Event): Promise<void> {
    ...
    if (!file) return;
    this.refFile = file; this.refFilename = file.name; this.refError = ''; this.refParsing = true;
    try {
      this.refPoints = await parseRefFile(file);
      if (this.refPoints.length < 2) throw new Error('La referencia necesita al menos 2 puntos');
      this.refIdx = buildRefIndex(this.refPoints);
      console.log(`[Ref] "${file.name}": ${this.refPoints.length} pts, ${this.refIdx.totalArc.toFixed(0)} m, ${this.refIdx.cornerIndices.length} corners`);
    } catch (e: any) {
      this.refError = e.message ?? 'Error al parsear el fichero de referencia';
      this.refPoints = []; this.refIdx = undefined;
    } finally { this.refParsing = false; }
  }
  ...
  onRefDragOver(e: DragEvent): void { e.preventDefault(); }
  ...
  removeMode(i: number): void { this.modes.splice(i, 1); }

  onFilesSelected(event: Event, mode: ModeSetup): void {
    mode.fileInputFiles = Array.from((event.target as HTMLInputElement).files ?? []).slice(0, 10);
    mode.error = '';
  }
  ...
  onDrop(event: DragEvent, mode: ModeSetup): void {
    ...
    mode.fileInputFiles = Array.from(event.dataTransfer?.files ?? [])
      .filter(f => f.name.toLowerCase().endsWith('.gpx')).slice(0, 10);
    mode.error = '';
  }
  ...
  canAnalyze(): boolean {
    return !this.analyzing && !!this.refIdx && this.modes.some(m => m.fileInputFiles.length > 0);
  }

  // ── Analysis ──────────────────────────────────────────────────────────────

  async analyze(): Promise<void> {
    if (!this.refIdx) { this.analyzeError = 'Primero sube la Línea de Verdad Absoluta (.gpx o .kml)'; return; }
    ...
    try {
      ...
      for (const setup of this.modes) {
        ...
        setup.loading = false;
        result.push(this.buildModeAnalytics(setup.id, setup.name, setup.color, runs));
      }
      if (result.length) {
  ...
  private buildModeAnalytics(id: string, name: string, color: string, runs: GpsRunFile[]): UrbanModeAnalytics {
    ...
    for (const run of runs) {
      ...
      if (idx.cornerIndices.length > 0) {
        const cornerPtsLocal = idx.cornerIndices.map(ci => [idx.xs[ci], idx.ys[ci]] as [number, number]);
        const gpsLocal = run.points.map(p => gpsToLocal(p.lat, p.lon, idx.origin.lat, idx.origin.lon));
        let sumCornerErr = 0;
        for (const [cx, cy] of cornerPtsLocal) {
          let minD = Infinity;
          for (const [gx, gy] of gpsLocal) { const d = Math.sqrt((gx-cx)**2+(gy-cy)**2); if (d < minD) minD = d; }
          sumCornerErr += minD;
        }
      ...
      console.log(
        ...
        idx.cornerIndices.length ? `corner=${runMetrics[runMetrics.length-1].cornerErr.toFixed(2)}m` : 'no corners',
      );
    }

    const mean = (arr: number[]) => { const v = arr.filter(x => !isNaN(x)); return v.length ? v.reduce((a,b)=>a+b,0)/v.length : NaN; };
    const sampleHz = (() => { const fp = runs[0]?.points.filter(p=>p.time)??[]; if (fp.length<2) return 0; const dt=(fp[fp.length-1].time!.getTime()-fp[0].time!.getTime())/1000; return dt>0?Math.round((fp.length/dt)*100)/100:0; })();

    return {
      modeId: id, modeName: name, color,
  ...
  private loadBuildings(): void {
    ...
    for (const m of this.analytics) {
      for (const run of m.runs) {
        for (const p of run.points) { lats.push(p.lat); lons.push(p.lon); }
      }
    }
    const pad = 0.0005;
    ...
    this.http.post<any>(
      ...
      next: data => {
        this.buildings = (data.elements ?? [])
          ...
          .map((el: any) => (el.geometry as any[]).map(p => ({ lat: p.lat, lon: p.lon })));
        this.ngZone.run(() => {
      ...
      error: () => { this.ngZone.run(() => { this.loadingBuildings = false; }); },
    });
  }
      ...
      for (let ri = 0; ri < m.runs.length; ri++) {
        ...
        for (const p of pts) {
          for (const poly of this.buildings) {
            if (pointInPolygon(p, poly)) { inside++; break; }
          }
        }
        m.runMetrics[ri].buildingPct = pts.length > 0 ? (inside / pts.length) * 100 : 0;
      }
      const valid = m.runMetrics.filter(r => !isNaN(r.buildingPct));
      m.buildingPct = valid.length ? valid.reduce((s, r) => s + r.buildingPct, 0) / valid.length : NaN;
    }
  }
  ...
  private initLeafletMap(container: HTMLDivElement): void {
    this.leafletMap = L.map(container, { zoomControl: true, scrollWheelZoom: true, preferCanvas: true });
  ...
  private drawAllLayers(): void {
    ...
    if (this.refPoints.length > 1) {
      this.refLayer = L.polyline(this.refPoints.map(p => [p.lat, p.lon] as [number, number]), {
        color: '#1a1a1a', weight: 5, opacity: 0.85,
      }).addTo(this.leafletMap);
      L.polyline(this.refPoints.map(p => [p.lat, p.lon] as [number, number]), {
      ...
      if (this.refIdx) {
        for (const ci of this.refIdx.cornerIndices) {
          const marker = L.circleMarker([this.refPoints[ci].lat, this.refPoints[ci].lon], {
            radius: 5, color: '#FF6B35', fillColor: '#FF6B35', fillOpacity: 0.9, weight: 2,
          }).addTo(this.leafletMap);
    ...
    for (const m of this.analytics) {
      ...
      for (const run of m.runs) {
        runLines.push(L.polyline(run.points.map(p => [p.lat, p.lon] as [number, number]), {
          color: m.color, weight: 2.5,
          opacity: m.visible ? (this.hoveredModeId && this.hoveredModeId !== m.modeId ? 0.12 : 0.75) : 0,
          smoothFactor: 1.2,
        }).addTo(this.leafletMap!));
      }
  ...
  private fitMapToAll(): void {
    ...
    for (const m of this.analytics ?? []) {
      for (const run of m.runs) all.push(...run.points.map(p => [p.lat, p.lon] as [number, number]));
    }
    if (all.length) this.leafletMap.fitBounds(L.latLngBounds(all), { padding: [24, 24] });
  }
  ...
  private updateMapOpacity(): void {
    ...
    for (const m of this.analytics ?? []) {
      ...
      this.runLayers.get(m.modeId)?.forEach(l => l.setStyle({ opacity: op, weight: hovered ? 4 : 2.5 }));
    }
  }
  ...
  loadTest(testId: string): void {
    this.api.getUrbanTest(testId).subscribe({
      next: (test: any) => {
        ...
        this.refIdx      = this.refPoints.length >= 2 ? buildRefIndex(this.refPoints) : undefined;

        if (!this.refIdx) {
          console.warn('[Urban] Test loaded without ref_points — cannot compute metrics');
          return;
        }

        const result: UrbanModeAnalytics[] = (test.modes ?? []).map((m: any) => {
          const runs: GpsRunFile[] = m.runs.map((r: any) => ({
            filename: r.filename, distance_m: r.distance_m,
  ...
  deleteTest(testId: string, event: Event): void {
    ...
    this.api.deleteUrbanTest(testId).subscribe({
      next: () => {
        ...
        if (this.currentTestId === testId) { this.analytics = null; this.currentTestId = null; }
      },
    });
  }
    ...
    const payload = {
      ...
      modes: result.map(m => ({
        ...
        runs: m.runs.map(r => ({
          ...
          points:     r.points.map(p => ({ lat: p.lat, lon: p.lon, ele: p.ele, time: p.time?.toISOString() })),
        })),
      })),
    };
    this.api.saveUrbanTest(this.deviceId, payload).subscribe({
      next: saved => { this.currentTestId = saved.id; this.savedTests = [saved, ...this.savedTests]; this.savingTest = false; },
  ...
  get sortedByRmse(): UrbanModeAnalytics[] {
    return [...(this.analytics ?? [])].sort((a, b) => {
      ...
      if (isNaN(a.rmse)) return 1; if (isNaN(b.rmse)) return -1;
      return a.rmse - b.rmse;
    });
  }

  selectMode(id: string): void { this.analytics?.forEach(m => m.selected = m.modeId === id); }

  toggleVisibility(id: string): void {
    const m = this.analytics?.find(a => a.modeId === id);
    if (m) { m.visible = !m.visible; this.ngZone.runOutsideAngular(() => this.updateMapOpacity()); }
  }
  ...
  urbanAptitude(m: UrbanModeAnalytics): UrbanAptitude {
    if (!this.refIdx) return { level: 'sin-ref', label: 'Sin referencia', sublabel: 'Carga una Línea de Verdad Absoluta para evaluar', failing: [] };
    const failing: string[] = [];
    if (!isNaN(m.buildingPct) && m.buildingPct > 0) failing.push(`${m.buildingPct.toFixed(1)}% puntos en edificios`);
    if (!isNaN(m.cornerErr) && m.cornerErr > 5) failing.push(`Error esquinas ${m.cornerErr.toFixed(1)} m`);

    if (m.rmse <= 2.5) return { level: 'elite',    label: 'Urban Elite',    sublabel: 'RMSE ≤ 2.5 m: sigue la acera con fidelidad milimétrica', failing };
    if (m.rmse <= 5)   return { level: 'high',     label: 'High Tier',      sublabel: 'RMSE ≤ 5 m: distancia correcta, leve redondeo en giros', failing: [...failing, `RMSE ${m.rmse.toFixed(2)} m (lím. Elite: 2.5 m)`] };
    if (m.rmse <= 10)  return { level: 'standard', label: 'Standard',       sublabel: 'RMSE ≤ 10 m: puede cruzar a la acera opuesta',           failing: [...failing, `RMSE ${m.rmse.toFixed(2)} m`] };
    if (m.rmse <= 15)  return { level: 'marginal', label: 'Marginal',       sublabel: 'RMSE 10–15 m: distorsiones severas en cañones urbanos',   failing: [...failing, `RMSE ${m.rmse.toFixed(2)} m`] };
    return                    { level: 'no-apto',  label: 'No Apto',        sublabel: 'RMSE > 15 m: atraviesa manzanas enteras',                 failing: [...failing, `RMSE ${m.rmse.toFixed(2)} m`] };
  }
  ...
  get chartPaths(): { d: string; color: string; dim: boolean }[] {
    ...
    for (const m of this.analytics) {
      ...
      for (const rm of m.runMetrics) {
        ...
        const pts = rm.errProfile.map(pt => `${this.chartX(pt.s).toFixed(1)},${this.chartY(pt.err).toFixed(1)}`);
  ...
  get chartThresholds(): { y: number; label: string; color: string }[] {
    return this.THRESHOLDS
      .filter(t => t.val <= this.chartMaxErr)
      .map(t => ({ y: this.chartY(t.val), label: t.label, color: t.color }));
  }
  ...
  get chartYTicks(): { y: number; label: string }[] {
    const ticks = [0, 5, 10, 15, 20].filter(v => v <= this.chartMaxErr);
    return ticks.map(v => ({ y: this.chartY(v), label: `${v}m` }));
  }

  // ── Helpers ───────────────────────────────────────────────────────────────

  fmt(v: number, dec = 1): string { return isNaN(v) || !isFinite(v) ? '—' : v.toFixed(dec); }
  fmtHz(v: number): string { return v > 0 ? `${v.toFixed(2)} Hz` : '—'; }
  fmtPct(v: number): string { return isNaN(v) ? '—' : `${v.toFixed(1)}%`; }
  fmtMs(v: number): string { return isNaN(v) ? '—' : `${(v * 100).toFixed(1)} cm/s`; }

  get totalRuns(): number { return (this.analytics ?? []).reduce((s, m) => s + m.runs.length, 0); }
  get refTotalArc(): number { return this.refIdx?.totalArc ?? 0; }
  get cornerCount(): number { return this.refIdx?.cornerIndices.length ?? 0; }
